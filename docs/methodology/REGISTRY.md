...
- **Note:** CallawaySantAnna survey weights compose with IPW weights multiplicatively: w_total = w_survey * p(X) / (1-p(X)). Propensity scores estimated via survey-weighted solve_logit(). WIF in aggregation uses survey-weighted group sizes. Bootstrap + survey deferred.
